...
function openPop(btn) {
  ...
  const prov = L.status !== 'confirmed' || (LANG === 'es' && L.es_status !== 'confirmed');
  pop = document.createElement('div'); pop.className = 'pop'; pop.setAttribute('role', 'dialog');
  pop.innerHTML = `<div class="l1"><span class="my" lang="my">(${esc(btn.dataset.l)})</span><span>·</span>${L.pali ? `<span class="pl" lang="pi">${esc(L.pali)}</span><span>·</span>` : ''}<span>${esc(meaning)}</span>${prov ? `<span class="prov">${esc(t('provisional'))}</span>` : ''}<a href="/labels/#l${L.k}" title="${esc(t('all_labels'))}" aria-label="${esc(t('all_labels'))}">↗</a></div>` +
